...
> Superseded on 2026-03-22. Latest guide: `docs/plans/2026-03-22-docker-deployment-guide.md`.
...
- `DOUYIN_QR_CHROME_PATH`

说明：
- 如果下载接口拿不到请求头里的 `host`，会尝试用 `PUBLIC_API_ORIGIN` 把相对 `/api/...` 下载链接补成绝对地址
- 如果 `PUBLIC_API_ORIGIN` 也未设置，则保持相对 `/api/...` 返回，不再写死 `localhost:3001`
...
6. Runtime 看板可正常请求 `/api/admin/runtime-dashboard` 与 `/api/admin/runtime-dashboard/chains*`。  
7. 下载模式切换后，Web/Mobile 的 `clientType` 分流行为符合预期。  
8. 当处于反向代理或容器外调用环境时，若需要绝对下载链接，已正确配置 `PUBLIC_API_ORIGIN`。  
...
6. 抖音代理 `ENETUNREACH`：
- 属于当前已知网络环境风险，后续仍需继续补充 IPv4 优先等兜底策略。
