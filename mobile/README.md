...
3. 相册兼容自动重试：
- 若 iOS 写入相册报不兼容（如 `PHPhotosErrorDomain 3301` / `IOS_PHOTOS_INCOMPATIBLE_CODEC`），前台下载入口统一走友好提示（顶部 Toast）并自动重试一次 `iosCompatible=true`
- 原生静默下载链路命中同类错误时，也会自动补一次 `iosCompatible=true` 重试后再决定最终成败
...
- 当前定义是“iOS 原生静默下载队列”：下载阶段使用原生 `background URLSession`，不是纯前台任务。
- `prepare` 与相册写入属于可恢复收口阶段；当前稳定性依赖 `background URLSession + 启动恢复 + saving 可恢复 + 一次性 iosCompatible 自动重试`，本次不接 `BGTask`。
- 冷启动时会先等待 auth store hydrate，再决定是否向原生下发可运行配置；登录态恢复后，`auth_required` 暂停会自动清除并继续队列。
- 相册权限缺失等权限类暂停会保留队列内容并持久化状态，等待用户处理后手动恢复。
- 本地持久化写入的是“可恢复快照”，不会把下载过程中的细碎进度持续落盘。
- 运行时队列与持久化历史都会保留有限条最近完成/失败任务，避免本地队列无限膨胀；当前完成/失败历史默认只保留最近 20 条。
- 历史与结果列表按真实结束时间优先排序；已完成/失败任务展示“完成/失败时间”，进行中任务仍展示入队时间。
- 若命中明显阻塞后续任务的错误（如相册权限缺失、登录态失效），队列会暂停并持久化该状态；其中登录态失效在重新登录后可自动恢复，权限类错误仍需手动恢复。
...
- `BGTask` 级后台补偿推进（prepare / saving 不依赖再次打开 App）
- 真正的断点续传
- 系统推送（APNs / FCM）
- 支付闭环
